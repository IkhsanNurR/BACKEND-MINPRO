...
import { route_actions } from "models/masterSchema";
...
export class RouteActionsService {
  ...
  async update(id: number, updateRouteActionDto: UpdateRouteActionDto) {
    try {
      const result = await route_actions.update(updateRouteActionDto, {
        where: { roac_id: id },
      });
      return result;
    } catch (error) {
  ...
  async updateDisplay(id: number, updateDisplayDto: UpdateDisplayDto) {
    try {
      const result = await route_actions.update(updateDisplayDto, {
        where: { roac_id: id },
      });
      return result;
    } catch (error) {
