...
import { client, job_photo, job_post, job_post_desc } from 'models/jobhireSchema';
...
export class JobHireService {
  ...
  async createJopo(fields: any, image: any): Promise<any> {
    ...
    try {
      ...
      const query = `CALL job_hire.createpostingjob('${JSON.stringify(
        data,
  ...
  async createClient(fields: any): Promise<any> {
    ...
    try {
      ...
      const query = `CALL job_hire.createclient('${JSON.stringify(
        data,
  ...
  async updateClient(id: number, fields: any): Promise<any> {
    ...
    try {
      ...
      const query = `CALL job_hire.updateclient('${id}','${JSON.stringify(
        data,
      )}', '${JSON.stringify(data1)}')`;
      const result = await this.sequelize.query(query);
