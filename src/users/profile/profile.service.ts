...
export class ProfileService {
  constructor(
    ...
    @InjectModel(phone_number_type)
    private readonly PhoneType: typeof phone_number_type,
    @InjectModel(users_address)
    private readonly UserAddress: typeof users_address,
    @InjectModel(users_education)
    private readonly UserEducation: typeof users_education,
    @InjectModel(users_experiences)
    private readonly UserExperiences: typeof users_experiences,
  ...
  async uploadResume(
    id: number,
    updateProfilDto: UpdateProfileDto,
    file: Express.Multer.File
  ) {
    try {
      const filename = path.basename(file.path);
      const newPath = `http://localhost:3001/users/resume/${filename}`;
  ...
  async changePassword(id: number, updateProfileDto: UpdateProfileDto) {
    try {
      ...
      const match = await bcrypt.compare(
        updateProfileDto.user_password,
        user.user_password
      );
      if (!match) throw new Error("Password Lama Salah");

      if (updateProfileDto.newPassword != updateProfileDto.retypePassword)
  ...
  async addAddress(id: number, updateProfileDto: UpdateProfileDto) {
    try {
      ...
      const res = await this.sequelize.query(
        "CALL users.addAddress(:entityId,:address1,:addressPostalCode,:addressCityId,:addressTypeId" +
          (replacements.address2 ? ",:address2" : "") +
          ")",
        {
          replacements,
        }
  ...
  async editAddress(id: number, updateProfileDto: UpdateProfileDto) {
    try {
      ...
      const res = await this.sequelize.query(
        "CALL users.editAddress(:addressId,:address1,:addressPostalCode,:addressCityId,:addressTypeId" +
          (replacements.address2 ? ",:address2" : "") +
          ")",
        {
          replacements,
