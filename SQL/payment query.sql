...
BEGIN
    ...
    SELECT entity_id INTO new_entity_id FROM result;

    INSERT INTO payment.bank (bank_entity_id, bank_code, bank_name)
    SELECT new_entity_id, bank_code, bank_name
    FROM json_to_recordset(data) bank (bank_code VARCHAR, bank_name VARCHAR);
END $$;
...
--fintech regenerate business_entity
CREATE OR REPLACE PROCEDURE payment.insertfintech(In data JSON)
LANGUAGE plpgsql
AS $$
DECLARE 
...
BEGIN
    ...
    IF usac_type_param = 'debet' OR usac_type_param = 'credit card' THEN
