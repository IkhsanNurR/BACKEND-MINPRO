...
export class section_detail_material extends Model<section_detail_materialAttributes, section_detail_materialAttributes> implements section_detail_materialAttributes {
    ...
    @Column({
    	...
    	defaultValue: Sequelize.literal("nextval('curriculum.section_detail_material_sedm_id_seq'::regclass)") 
    })
    @Index({
    ...
    @Column({
    	allowNull: true,
    	type: DataType.INTEGER,
    	defaultValue: Sequelize.literal("nextval('curriculum.section_detail_material_sedm_secd_id_seq'::regclass)") 
    })
    	sedm_secd_id?: number;
